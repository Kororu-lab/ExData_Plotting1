...
##Changing datatype for $Date/$Time
data2[,1] <- as.Date(data2$Date, "%d/%m/%Y")
data2[,2] <- sub(".*\\s+", "",  strptime(data2$Time, "%H:%M:%S", tz = "UTC"))
...
with(data2, hist(as.numeric(GlobalActive), xlab = "Global Active Power (kilowatts)", ylab = "Frequency", main = "Global Active Power", col = "red"))
dev.off()
